...
### <span>**通联数据股份公司**  南京</span> <span>2025.01 -- 至今</span>

*数据业务部，高级软件开发工程师* 负责高频数据转发系统 Market Data Line（MDL）的日常研发及维护。

- 深交所逐笔合并。实时将深交所逐笔委托和逐笔成交数据按照业务序号合并排序，生成有序的完整逐笔行情流。较原本两路行情，延迟增加约 20ms.
- 上海黄金交易所数据接入及衍生分钟线开发。通过上金所 SDK 程序接收原始行情，将数据解析为 MDL 消息流，接入内部系统，形成数据服务对外提供。基于已有框架，按业务规则开发上金所分钟线。
- 项目代码重构。前期学习代码期间，提出改进点获架构师认可。删除冗余代码 1000+ 行，拆分模块代码 6000+ 行。
- 改进项目发布流程。集成 jenkins 平台，编写 shell 及 python 脚本完成自动编译、打包、发版流程。减少人工出错率，大幅缩短发版耗时。
- 运维工作：熟练部署 MDL 服务；redis 单点、集群；predixy 代理；haproxy 负载均衡等。
- 客户对接：技术支持，路演支持，方案建议，客户/售前/售后答疑等。

### <span>**华为技术有限公司**  南京</span> <span>2023.04 -- 2024.07</span>
...
- 了解 gdb 基本用法，会用 gdb 进行简单调试及问题定位
- 熟悉 Python 脚本语言，有相关开发经验
- 熟悉 shell 脚本，可将重复的工作自动化
- 熟悉 git、svn 两种版本管理工具
- 熟悉常用的 Linux 命令，PC 使用 Linux 系统已有 10 年余，有一定的 Linux 基础
- 通过国家英语六级，有较好的英文阅读能力
